    ...
    try:
        ...
        if str(sys.argv[5])=="txt":
            ...
            if len(inp.columns)==1:
                ...
                inp = inp.to_csv(header=None, index=False).strip("\n").split("\n")
                inp = ",".join(inp)
            else:
                #inp = inp.to_string(header=False, index=False).replace(" ",",")
                inp = inp.to_csv(header=None, index=False).strip(sep).split(sep)
                inp = ",".join(inp)
...
def get_genes(inp,g_out):
    ...
    ## All the data is paginated using the page and size parameters, by default the endpoint returns the first 20 
hits
    gene_by_phenotypes_query = 
f"{impc_api_search_url}/search/findAllBySignificantMpTermIdsContains?mpTermIds={target_mp_terms}&page=0&size=20"
    ...
    if g_out == "sym":
        list_of_genes = pd.DataFrame(columns=['Gene symbol id', 'Gene name', 'Gene bundle url'])
...
def gene_set(inp):
    ...
    genes_in_gene_list_query = 
f"{impc_api_search_url}/search/findAllByMgiAccessionIdIn?mgiAccessionIds={target_genes}"
    ...
    for gene in genes_in_gene_list['_embedded']['genes']:
        ...
        for mp_term_name in mp_terms:
            if mp_term_name['mpTermId'] not in list_of_mp_terms_vs_gene_index:
                list_of_mp_terms_vs_gene_index[mp_term_name['mpTermId']] = {"mp_term": mp_term_name['mpTermId'], 
...
def sign_par(inp):
    ...
    gene_info = 
requests.get(f"{impc_api_url}statisticalResults/search/findAllByMarkerAccessionIdIsAndSignificantTrue?mgiAccessionId=" 
+ knockout).json()
...
def genes_in_pipeline(inp,g_out):
    ...
    if max_elem > 1000:
        ...
        for i in range(1,pages):
            gl = 
requests.get(f'{impc_api_search_url}/search/findAllByTestedPipelineId?pipelineId={pip}&page={i}&size=1000').json()['_embedded']['genes']
            g_in_p_query += gl
    else:
        g_in_p_query = genes_in_pip['_embedded']['genes']
...
def sign_mp(inp,g_out):
    ...
    gene_by_mpterm_query = 
f"{impc_api_search_url}/search/findAllBySignificantMpTermIdsContains?mpTermIds={mp_term}&size=1000"
    ...
    for pn in range(1,pages):
        pq = 
f"{impc_api_search_url}/search/findAllBySignificantMpTermIdsContains?mpTermIds={mp_term}&page={pn}&size=1000"
...
def full_gene_table(g_out):
    ...
    for i in genes_info:
        ...
        if i['significantMpTerms'] is None:
            ...
            d = {"Gene": i['mgiAccessionId'], "Identified phenotypes": [sub['mpTermId'] for sub in 
i['significantMpTerms']]}
        list_d.append(d)
...
def par_pip_ma(inp):
    ...
    if max_elem > 1000:
        ...
        for pn in range(1, pages):
            if par:
                pip = 
requests.get(f"{impc_api_search_url}/search/findAllByTestedParameterId?parameterId={id}&page={pn}&size=1000").json()['_embedded']['genes']
            else:
                pip = 
requests.get(f"{impc_api_search_url}/search/findAllByTestedPipelineId?pipelineId={id}&page={pn}&size=1000").json()['_embedded']['genes']
            ma_in_pip += pip
...
requests.get(f"{impc_api_search_url}/search/findAllByTestedParameterId?parameterId={id}&page={pn}&size=1000").json()['_embedded']['genes']
    ...
    if g_out=="sym":
        list_of_genes = pd.DataFrame(columns=['Gene symbol', 'Measured Values'])
        list_of_genes['Gene symbol'] = mgi_sym_map(gen)
    else:
        list_of_genes = pd.DataFrame(columns=['Gene accession id', 'Measured Values'])
...
def mgi_sym_map(mgi_list):
    sym_list = []
    mg = mygene.MyGeneInfo()
    ginfo = mg.querymany(mgi_list, scopes='MGI', fields="symbol,MGI", species='mouse')
    discarded=[]
    for i in ginfo:
        try:
