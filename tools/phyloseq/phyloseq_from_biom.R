...
option_list <- list(
    ...
    make_option(c("--parseFunction"), action = "store", dest = "tree", default = "parse_taxonomy_default", help = "Parse function parse_taxonomy_default/read_tree_greengenes"),
...
parseFunction <- eval(opt$parseFunction)
phyloseq_obj <- import_biom(opt$biom, opt$treefilename, opt$refseqfilename, parseFunction=parseFunction)
