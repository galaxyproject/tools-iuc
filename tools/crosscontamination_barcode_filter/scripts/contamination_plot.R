...
contaminationPlot <- function(title, columndata, barcode.data, plate.data, RAW) {
    ...
    drawPlates <- function(plate.data) { # 0 384 768
        ...
        gplot <<- gplot + geom_segment(
            aes(
                x = min(unname(unlist(plate.boundaries))), xend = max(unname(unlist(plate.boundaries))),
                y = plate.minyval, yend = plate.minyval
            ),
            color = plate.color, lty = 1, size = plate.size,
        ...
        zzz <- lapply(names(plate.boundaries), function(plate.name) {
            ...
            if (plate.num > 0) {
                ...
                gplot <<- gplot + annotate("text",
                    x = p.xval, label = paste("Plate", plate.num),
