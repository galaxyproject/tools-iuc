...
my (@files, $fni, $fastq, $dir, $raw, $rawSens, $rawReverse, $rawU, $zip, $fin,$frej, $ref,$proc, $fout, $funi, $sam, $dup, $mis, $help, %genome_hits, %duplicates, $repartition, $png_rep, $rep_size,$aligner, $noD);
$mis = 3;
...
if (defined ($dir))
{
    ...
    $dir = $dir.'/' unless $dir =~ /(.*)\/$/;
    mkdir $dir;
    $fout = $dir.$name.'_'.$name_r.'.fastq';
    push(@files, $fout);
    if ($funi)
    {
        $fni = $dir.$name.'_unique_'.$name_r.'.fastq'; 
        push(@files, $fni);}else {$fni = '/dev/null';
    }
    $frej = $dir.$name.'_rejected_'.$name_r.'.fastq';
    push(@files, $frej);
    $sam = $dir.$name.'_'.$name_r.'_sorted.sam';
    push(@files, $sam);
    $raw = $dir.$name.'_'.$name_r.'_reads_counts.txt';
    push(@files, $raw);
    $rawU = $dir.$name.'_'.$name_r.'_unique_reads_counts.txt';
    push(@files, $rawU);
    $rawSens = $dir.$name.'_'.$name_r.'_reads_counts_sens.txt';
    push(@files, $rawSens);
    $rawReverse = $dir.$name.'_'.$name_r.'_unique_reads_counts_reverse.txt';
    push(@files, $rawReverse);
    if ($repartition)
    {
        $repartition = $dir.$name.'_'.$name_r.'_distribution.txt';
        push(@files, $repartition);
...
my @return = align($ref, $mis, $fin, $fout, $fni, $frej, $sam, \%duplicates, \%genome_hits, $proc);
my $uniqueNum  = $return[6];
my $num = $return[5];
my $size_mappedHashR = $return[-4];
my $size_mapped_spe_HashR = $return[-3];
print $num."\t".$uniqueNum."\n";
...
if ($repartition)
{
    ...
    open (my $rep, '>'.$repartition) or die "Cannot open $repartition align_stranded: $!\n";
    ...
    unless ($noD)
    {
        ...
        foreach my $k (keys %{$size_mapped_spe_HashR})
        {
            ...
            open ($rep, '>'.$repartition) or die "Cannot open $repartition align stranded: $!\n";
            $total = 0;
            ...
            histogram($size_mapped_spe_HashR->{$k}, $png_rep, $total);
        }
    }
}

$ma = $return[5] if $ma == 0;
rpms_rpkm($return[0], $return[4], $ma, $raw, $pi, $mi, $bo);
rpms_rpkm($return[1], $return[4] ,$ma,  $rawU, $pi, $mi, $bo) if ($funi);
rpms_rpkm($return[2], $return[4] ,$ma,  $rawSens, $pi, $mi, $bo) ;
rpms_rpkm($return[3], $return[4] ,$ma,  $rawReverse, $pi, $mi, $bo) ;
...
if (defined $dup)
{
    ...
    #print STDERR ("DEBUG_align_count_stranded_dup:".$dup."\n");
...
{
    my $zipfile = $1 if $dir =~ /(.*)\/$/;
    #print STDERR ("DEBUG_align_count_stranded_zipfile:".$zipfile."\n");
