    ...
    tryCatch(
        {
            regionsFeatures <- IWTomicsData(regionspaths, featurespaths, alignment,
                id_regions, name_regions, id_features, name_features,
                start.are.0based = start.are.0based
            )
        },
        error = function(err) {
        ...
        tryCatch(
            {
                ...
                if (alignment == "scale") {
                    if (scale == 0) {
                        regionsFeatures <- smooth(regionsFeatures,
                            type = smoothing, fill_gaps = fill_gaps,
