...
def main():
    ...
    with open(os.path.join(model_dir, "final.mdl"), "wb") as f: f.write(b"DUMMY")
    with open(os.path.join(model_dir, "tree"), "wb") as f: f.write(b"DUMMY")
    create_zip("dummy_acoustic_model.zip", model_dir)
    shutil.rmtree(model_dir)

    # 8. Create 'test_corpus_aligned.zip' (FIXED HEADER)
    aligned_dir = "temp_aligned"
    ensure_dir(os.path.join(aligned_dir, "s1"))
    create_sine_wave(os.path.join(aligned_dir, "s1", "1.wav"))
    
    # FIX IS HERE: Added blank line after Object class
...
item []: 
    item [1]: 
        ...
        intervals [1]:
            ...
            text = "HH" 
"""
    with open(os.path.join(aligned_dir, "s1", "1.TextGrid"), "w") as f: f.write(tg_content)
    create_zip("test_corpus_aligned.zip", aligned_dir)
    shutil.rmtree(aligned_dir)

    # 9. Create 'dummy_ipa_model.zip'
    ...
    with open(os.path.join(ipa_model_dir, "final.mdl"), "wb") as f: f.write(b"DUMMY")
    with open(os.path.join(ipa_model_dir, "tree"), "wb") as f: f.write(b"DUMMY")
