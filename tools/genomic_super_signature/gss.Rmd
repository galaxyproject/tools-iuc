...
Here, we print out top `r numOut` validated RAVs with average silhouette 
width above 0.

```{r out.height="45%", out.width="45%", message=FALSE, warning=FALSE}
heatmapTable(val_all, num.out = numOut, swCutoff = 0)
```
...
for (i in seq_len(n)) {
    RAVnum <- validated_ind[i]
    res <- findStudiesInCluster(RAVmodel, RAVnum, studyTitle = TRUE)
    res_all[[i]] <- head(res)
