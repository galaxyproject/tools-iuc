...
class FrequencyDistribution(object):
    ...
    def get_bin(self, x):
        ...
        return (
            self.start
            + (x - self.start) // self.binsize * self.binsize
            + self.binsize / 2.0
        )
...
def gff_row(
    cname, start, end, score, source, type=".", strand=".", phase=".", attrs={}
):
...
def frequency_plot(freqs, fname, labels=[], title=""):
    ...
    pyplot.subplots_adjust(
        left=ADJUST[0], right=ADJUST[1], top=ADJUST[2], bottom=ADJUST[3]
    )
...
def perform_process(
    ...
    output_orphans = output_files in [
        "all",
        "matched_pair_orphan",
        "matched_pair_orphan_detail",
    ]
    ...
    def make_histogram_path(output_type, fname):
        return os.path.join(
            HISTOGRAM, "histogram_%s_%s.%s" % (output_type, fname, PLOT_FORMAT)
        )
    ...
    def td_writer(output_type, extension, fname):
        ...
        return csv.writer(
            open(output_file_path, "wt"), delimiter="\t", lineterminator="\n"
        )
    ...
    with open(dataset_path, "rt") as input:
        try:
            ...
            stop_err(
                ...
                % (dataset_path, traceback.format_exc())
            )
    if output_details:
        # Details
        detailed_output = td_writer("data_%s" % DETAILS, TABULAR_EXT, fname)
        detailed_output.writerow(
            ("chrom", "start", "end", "value", "strand") * 2
            + ("midpoint", "c-w reads sum", "c-w distance (bp)")
        )
    if output_plots:
        # Final Plot
