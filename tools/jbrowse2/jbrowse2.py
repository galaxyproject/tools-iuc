...
class JbrowseConnector(object):
    ...
    def process_genomes(self):
        for genome_node in self.genome_paths:
            ...
            shutil.copy(genome_node['path'], copied_genome)
    ...
    def add_bam(self, data, trackData, bamOpts, bam_index=None, **kwargs):
        rel_dest = os.path.join('data', trackData['label'] + '.bam')
        dest = os.path.join(self.outdir, rel_dest)
        print("exists in bam ? %s %s %s %s" % (os.path.realpath('data'), os.path.exists(os.path.realpath('data')), os.path.realpath(dest), os.path.exists(os.path.realpath(dest))))
        self.symlink_or_copy(os.path.realpath(data), dest)
