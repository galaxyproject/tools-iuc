...
# Aims: Convert tsv blast files into abundance files for use by krona
...
def _create_abundance(options):
    ...
    i = options.input[0]
    file_path = i
    file_name = file_name = os.path.basename(i)
    ...
    for samp in abundance:
        reads_file = open("krona_reads.tsv", "w+")
        abundance_files.append("krona_reads.tsv")
        for taxo in abundance[samp]:
            ...
            reads_file.write("\n")
        reads_file.close()
        contigs_file = open("krona_contigs.tsv", "w+")
        abundance_files.append("krona_contigs.tsv")
        for taxo in abundance[samp]:
            ...
            contigs_file.write("\n")
        contigs_file.close()
        log.debug("Abundance files created")


def _set_options():
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input', help='tsv delimited file with results of blast from blast2ecsv',
                        action='append', required=True, dest='input')
