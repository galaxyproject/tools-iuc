...
def __main__():
    ...
    parser.add_argument(
        "-k", "--knockout_type", dest="knockout_type", action="store", type=str, default="single", required=False, help="Type of knockout to perform, single or double"
    )
    parser.add_argument(
        "-g", "--gene_knockouts", dest="gene_knockouts", action="store", type=str, default=None, required=False, help="List of genes to knock out. Defaults to all."
    )
    parser.add_argument(
    ...
    genes_to_knockout_1 = args.gene_knockouts.split(',') if args.gene_knockouts is not None else []
    gene_bool = [True if gene in gene_ids else False for gene in genes_to_knockout_1]
    if not all(gene_bool):
        print(f'Found {sum(gene_bool)} of {len(genes_to_knockout_1)} genes in the model.')
        raise Exception("One or more of the genes to knockout are not present in the model.")
    ...
    result = pd.DataFrame(columns=["reaction_id", "ko_gene_id_1", "ko_gene_id_2", "reaction", "wildtype_flux", "knockout_flux"])
    ...
    if args.knockout_type == "single":
        ...
        raise Exception(f"Invalid knockout type {args.knockout_type}. Only single and double are allowed.")
            ...
            with cb_model as model:
                ...
                for reaction in model.reactions:
                    result = pd.concat([result, pd.DataFrame([{
                        ...
                        "ko_gene_id_1": gene1,
                        "ko_gene_id_2": gene2 if args.knockout_type == "double" else None,
