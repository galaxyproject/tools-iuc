...
def main():
    ...
    if not (0 <= lab_idx < ncols) or not (0 <= count_idx < ncols):
        print(
            f"ERROR: column index out of range. File has {ncols} columns.",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    x = count_column + "_x"
    y = count_column + "_y"
    cross = df[[xy_column, count_column]].merge(
        df[[xy_column, count_column]], how="cross"
    )

    cross["intersection"] = cross.apply(
        lambda row: row[x].intersection(row[y]), axis=1
    ...
    y = xy_column + "_y"
    mat = (
        cross.pivot(index=x, columns=y, values="intersection")
    ...
    for i in mat:
        c_mat[i] = (
            ...
            .astype(str)
            .str.split(",")
            .apply(lambda xs: len([x.strip() for x in xs if x and x.strip()]))
        )
