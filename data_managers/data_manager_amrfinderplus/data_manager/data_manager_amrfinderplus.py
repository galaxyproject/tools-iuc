...
class GetDataManager:
    ...
    def update_amrfinderplus_db(self, path):
        amrfinderplus_db_path = path.joinpath(self._db_name)
        cmd = [
            'amrfinder_update',
        ...
        if(proc.returncode != 0):
            print(f"ERROR: AMRFinderPlus failed! command: 'amrfinder_update --force_update --database {amrfinderplus_db_path}', error code: {proc.returncode}")
        else:
...
def main():
    ...
    amrfinder_output = amrfinderplus_download.update_amrfinderplus_db(amrfinderplus_download._db_path)
