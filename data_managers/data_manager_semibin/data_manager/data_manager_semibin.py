...
def download_gtdb(data_tables, table_name, target_dp, test=False, mmseq=False):
    """Download GTDB
    ...
    if test:
        ...
        if mmseq:
            cmd = "mmseqs databases GTDB %s/GTDB tmp" % (db_dp)
            subprocess.check_call(cmd, shell=True)
            version_cmd = "head -n 1 tmp/latest/version"
            version = subprocess.check_output(version_cmd, shell=True, text=True).strip()
            dbkey = 'gtdb'
            name = "GTDB reference genome generated by MMseqs2 used in SemiBin %s" % (version)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mmseq', action='store_true', help="MMseqs2 selected")
