...
def bracken_build_database(
    ...
    database_path = os.path.join(
        bracken_build_args["kraken_database"],
        "database" + str(bracken_build_args["read_len"]) + "mers.kmer_distrib",
    )

    if not prebuilt:
        bracken_build_args_list = [
            "-t",
            bracken_build_args["threads"],
            "-k",
            bracken_build_args["kmer_len"],
            "-l",
            bracken_build_args["read_len"],
            "-d",
            bracken_build_args["kraken_database"],
        ]
...
def main():
    ...
    parser.add_argument(
        "--prebuilt", action="store_true", dest="prebuilt", help="Use pre-built DB"
    )
